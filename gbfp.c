...
static void parseSource(FILE *FSeqFile, gb_data *ptGBData) {
    ...
    /* Some malformed genbank flat files do not have a Lineage line!
     * To make sure this doesn't disrupt parsing (joinLines will eat too much)
     * we need to check ahead that there is a lineage line otherwise we are 
     * skipping the lineage line altogether. Should we give a warning?
     */
    getLine(sLine, FSeqFile);
    putLine(sLine);
    debug_print(("sLine: %s\n", sLine));    
    if (memcmp(sLine, "            ", 12) == 0) {  
        /* We appear to have a lineage line, so we consume the lines */
        ptGBData->sLineage = joinLines(FSeqFile, 12);
    }

    debug_print(("Source: %s\n", ptGBData->sSource));
    debug_print(("Organism: %s\n", ptGBData->sOrganism));
    debug_print(("Lineage: %s\n", ptGBData->sLineage));
}
...
static void parseReference(FILE *FSeqFile, gb_data *ptGBData) {
    ...
    debug_print(("  AUTHORS  %s\n", ptReference->sAuthors));
    debug_print(("  CONSRTM  %s\n", ptReference->sConsrtm));
    debug_print(("  TITLE  %s\n",   ptReference->sTitle));
    debug_print(("  JOURNAL  %s\n", ptReference->sJournal));
    debug_print(("  MEDLINE  %s\n", ptReference->sMedline));
    debug_print(("  PUBMED  %s\n",  ptReference->sPubMed));
    debug_print(("  REMARK  %s\n",  ptReference->sRemark));
}
